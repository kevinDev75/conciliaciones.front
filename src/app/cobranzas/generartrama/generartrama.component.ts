...
@Component({
  ...
  templateUrl: './generartrama.component.html',
  styleUrls: ['./generartrama.component.scss'],
  providers: [
...
export class GenerarTramaComponent implements OnInit {
  ...
  public trama = new Array<{
    numeroCuenta: number, nombreContratante: string, documento: string, codigoProforma: string,
    fechaVencimiento: string, fechaEmision: string, importe: string, importeMora: string, importeMontoMinimo: string
  }>();
  ...
  // displayedColumnsTrama = ['CuentaProtecta', 'Contratante', 'Ruc', 'Cod. Proforma', 'Fecha Emisión', 'Fecha Vencimiento', 'Importe', 'Mora', 'Monto Mínimo'];
  displayedColumnsTrama = ['Tipo Doc', 'Ruc', 'Contratante', 'Cod. Proforma', 'N° Recibo', 'Fecha Emisión', 'Fecha Vencimiento', 'Importe'];
  ...
  onExportarExcel() {
    ...
    if (this.listaplanilla.length > 0) {
      this.serviceExporta.exportAsExcelFile(this.listaplanilla, 'Gestión de Cobranzas');
  ...
  onGenerarTrama() {
    if (this.fileTrama) {
      var file = new File([this.obtenerBlobFromBase64(this.fileTrama, '')], "TramaEnvio.txt", { type: "text/plain;charset=utf-8" });
      FileSaver.saveAs(file);
    } else {
      this.toastr.warning('No se pudo generar la trama', 'Gestión de Cobranzas');
    }
  ...
  cargarTabla() {
    ...
    buscarPlanilla.FechaInicial = datePipe.transform(this.fechaInicio, 'dd/MM/yyyy');
