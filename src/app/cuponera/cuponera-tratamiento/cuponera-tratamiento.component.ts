...
export class CuponeraTratamientoComponent implements OnInit {
  ...
  onCreateCupon() {
    ...
    this.service.GenerarCupon(params).subscribe(
      (s) => {
        ...
        if (s.p_NCODE === 0) {
          this.toastr.success(
            "Se genero correctamente el cupon N° " + String(s.data),
            "Gestion de Cuponera"
          );
        } else {
          this.toastr.error(
            "Hubieron problemas al generar el cupon",
            "Gestion de Cuponera"
          );
        }
      },
  ...
  onObtenerInfoCuponCab() {
    ...
    this.service.ObtenerInfoCupon(params).subscribe(
      (s) => {
        ...
        if (s.p_NCODE === 0) {
          ...
          this.service.ObtenerInfoCuponDetail(params).subscribe(
            ...
            (e) => {
              this.toastr.error(
                "Hubieron problemas al obtener el detalle de la cuponera",
                "Gestión de cuponera"
              );
            }
          );
      ...
      (e) => {
        this.toastr.error(
          "Hubieron problemas al buscar el cupon",
          "Gestión de cuponera"
        );
      }
    );
  ...
  onProcesarInfoPreview() {
    ...
    this.service.ObtenerPreviewCupon(params).subscribe(
      ...
      (e) => {
        this.toastr.error(
          "Hubieron problemas para generar la Factura",
          "Generar Fectura"
        );
      }
    );
  ...
  onBuscarInfoComprobante() {
    ...
    this.service.ObtenerIfoRecibo(params).subscribe(
      ...
      (e) => {
        this.toastr.error(
          "Hubieron problemas al buscar el recibo",
          "Gestión de cuponera"
        );
      }
    );
